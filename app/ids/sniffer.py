...
def _maybe_flush_connections(app):
    ...
    def _do_flush():
        from app.models import IPConnection, AppTrafficStat
        from app import db
        with app.app_context():
            try:
                for (src_ip, hostname), data in snapshot.items():
                    stat_date = data['last_seen'].date()
                    ...
                    daily_stat = AppTrafficStat.query.filter_by(
                        stat_date=stat_date,
                        source_ip=src_ip,
                        hostname=hostname,
                    ).first()
                    if daily_stat:
                        daily_stat.bytes_total += data['bytes']
                        daily_stat.packets_count += data['packets']
                        daily_stat.last_seen = data['last_seen']
                        if not daily_stat.app_name and data['app_name']:
                            daily_stat.app_name = data['app_name']
                    else:
                        daily_stat = AppTrafficStat(
                            stat_date=stat_date,
                            source_ip=src_ip,
                            hostname=hostname,
                            app_name=data['app_name'],
...
def _cleanup_old_app_traffic_stats(app, retention_days=None):
    """Șterge agregările zilnice per aplicație/site mai vechi decât pragul configurat."""
    from app import db
    from app.models import AppTrafficStat

    if retention_days is None:
        retention_days = app.config.get('APP_TRAFFIC_RETENTION_DAYS', 180)
    try:
        retention_days = int(retention_days)
    except (TypeError, ValueError):
        retention_days = 180
    ...
    threshold_date = (datetime.utcnow() - timedelta(days=retention_days)).date()

    try:
        with app.app_context():
            deleted = (
                AppTrafficStat.query
                .filter(AppTrafficStat.stat_date < threshold_date)
                .delete(synchronize_session=False)
            )
            if deleted:
                db.session.commit()
                print(f"[Sniffer] Curățare AppTrafficStat: {deleted} intrări mai vechi de {retention_days} zile șterse.")
            else:
                db.session.rollback()
            return deleted
    except Exception as e:
        print(f"[Sniffer] Eroare la curățarea AppTrafficStat: {e}")
        try:
            with app.app_context():
                db.session.rollback()
        except Exception:
...
def _calc_online_hours(device) -> float | None:
    """Calculează numărul de ore cât un dispozitiv a stat online (first_seen → last_seen).

    Args:
        device: Obiect NetworkDevice cu câmpurile first_seen și last_seen.
    Returns:
        Număr de ore ca float, sau None dacă datele lipsesc.
    """
    if device.first_seen and device.last_seen:
        delta = device.last_seen - device.first_seen
...
def _detect_device_type(ip_str, mac=None, vlan_id=None, hostname=None, online_hours=None):
    ...
    # Excepție: dacă dispozitivul a stat online ≥6 ore, probabil e un AP/server fix, NU mobil.
    if _is_randomized_mac(mac) and _get_vlan_from_ip(ip_str) is not None:
        if online_hours is None or online_hours < 6:
            return 'mobile'
        ...
        with app.app_context():
            ...
            for device in devices:
                ...
                if device.device_type in _FIXED_DEVICE_TYPES:
                    continue
                # Sări peste dispozitivele cu tip blocat manual de admin
                ...
                if device.vlan is not None:
                    try:
                        ...
                        pass
                # Calculăm durata online pentru heuristica de stabilitate
...
def _mobile_reset_scheduler(app):
    ...
    - _cleanup_old_app_traffic_stats: aplică retenția pentru agregările zilnice pe aplicație
    ...
    while _running:
        ...
        try:
            ...
            _cleanup_old_app_traffic_stats(app)
...
def start_sniffer(app):
    ...
    _cleanup_old_ip_connections(app)
    # Curățăm și agregările zilnice vechi pe aplicație/site
    _cleanup_old_app_traffic_stats(app)
